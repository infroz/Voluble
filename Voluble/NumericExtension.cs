...
public static class NumericExtension
{
    ...
    /// </summary>
    public static void BePositive<TNumber>(this TNumber value) where TNumber : INumber<TNumber>
    {
        if (!TNumber.IsPositive(value) || TNumber.IsZero(value))
            VolubleScope.FailWith($"Expected {value} to be positive");
    }

    /// <summary>
    /// Asserts that the number is negative (less than zero).
    /// </summary>
    public static void BeNegative<TNumber>(this TNumber value) where TNumber : INumber<TNumber>
    {
        if (!TNumber.IsNegative(value) || TNumber.IsZero(value))
            VolubleScope.FailWith($"Expected {value} to be negative");
    }

    /// <summary>
    /// Asserts that the number is zero.
    /// </summary>
    public static void BeZero<TNumber>(this TNumber value) where TNumber : INumber<TNumber>
    {
        if (!TNumber.IsZero(value))
            VolubleScope.FailWith($"Expected {value} to be zero");
    }

    /// <summary>
    /// Asserts that the number is not zero.
    /// </summary>
    public static void NotBeZero<TNumber>(this TNumber value) where TNumber : INumber<TNumber>
    {
        if (TNumber.IsZero(value))
            VolubleScope.FailWith($"Expected {value} to not be zero");
    }
    ...
    /// </summary>
    public static void BeInRange<TNumber>(this TNumber value, TNumber min, TNumber max)
        where TNumber : IComparable<TNumber>
    {
        if (value.CompareTo(min) < 0 || value.CompareTo(max) > 0)
            VolubleScope.FailWith($"Expected {value} to be in range [{min}, {max}]");
    }

    /// <summary>
    /// Asserts that the number is outside the specified range.
    /// </summary>
    public static void NotBeInRange<TNumber>(this TNumber value, TNumber min, TNumber max)
        where TNumber : IComparable<TNumber>
    {
        if (value.CompareTo(min) >= 0 && value.CompareTo(max) <= 0)
            VolubleScope.FailWith($"Expected {value} to not be in range [{min}, {max}]");
    }
    ...
    /// </summary>
    public static void BeApproximately(this double value, double expected, double tolerance)
    {
        if (double.IsNaN(value) || double.IsNaN(expected))
        {
            if (!(double.IsNaN(value) && double.IsNaN(expected)))
                VolubleScope.FailWith($"Expected {value} to be approximately {expected} (±{tolerance})");
            return;
        }
    ...
    /// </summary>
    public static void BeApproximately(this float value, float expected, float tolerance)
    {
    ...
    /// </summary>
    public static void BeApproximately(this decimal value, decimal expected, decimal tolerance)
    {
        if (Math.Abs(value - expected) > tolerance)
            VolubleScope.FailWith($"Expected {value} to be approximately {expected} (±{tolerance})");
    }
